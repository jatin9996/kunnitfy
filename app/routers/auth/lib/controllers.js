...
controllers.checkIfAuthorized = (req, res) => {
  if (req.userId) {
    return res.reply(messages.success("Success"), true)
  }
  return res.reply(messages.success(""), false)
}
...
controllers.login = (req, res) => {
  try {
    ...
    User.findOne({ sWalletAddress: _.toChecksumAddress(req.body.sWalletAddress) }, (err, user) => {
      if (err) return res.reply(messages.error());
      if (!user) {
        try {
          bcrypt.hash(req.body.sWalletAddress, saltRounds, (err, hash) => {
            ...
            user.save().then((result) => {
              let token = signJWT(user);
              res.cookie('decrypt_NFT_Token', token,
                {
                  domain: process.env.COOKIE_IP,
                  httpOnly: true,
...
controllers.adminlogin = (req, res) => {
  try {
    ...
    console.log("s email is--->", req.body.sEmail)
